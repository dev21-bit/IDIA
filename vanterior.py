...
if not st.session_state.autenticado:
    with st.container():
        ...
        with st.form("pin_form"):
            ...
            with pin_col1:
                pin = st.text_input("Ingresa tu clave de acceso", type="password", placeholder="Ej: IDAI1234", key="input_pin")
            with pin_col2:
                submit_pin = st.form_submit_button("Validar", use_container_width=True, key="submit_pin")
            if submit_pin:
...
if st.session_state.autenticado:
    ...
    with st.expander("📝 Registro de Nueva Credencial INE", expanded=True):
        ...
        if st.button("🧹 Limpiar Formulario", key="btn_limpiar_formulario"):
            reset_form()
        ...
        with st.form("form_ine"):
            ...
            if submit_ine:
                if not aceptar_terminos:
                    st.error("❌ Debes aceptar los Términos y Condiciones y el Aviso de Privacidad para continuar.")
